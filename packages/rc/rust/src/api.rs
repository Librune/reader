...
pub use book_core::{BookDetail, CatalogVolume, Chapter, MetaData, SearchBook};
...
#[frb]
pub fn get_code_metadata(code:String)->Result<MetaData, String>  { 
    let mut core = BookCore::init(code);
    core.get_metadata()
}

#[frb]
pub fn run_core_action(code:String,action:String,envs:String)->Result<String, String>  { 
    let mut core = BookCore::init(code);
    core.set_envs(json!(envs))?;
    core.run_action(action)?;
    core.get_envs().map(|v| v.to_string())
}

#[frb]
pub fn core_search_books(code:String,key: String, page: u8, count: u8)->Result<Vec<SearchBook>, String>  { 
    let mut core = BookCore::init(code);
    core.search_books(
        key,
        page,
...
#[frb]
pub fn core_book_detail(code:String,bid: String) -> Result<BookDetail, String> {
    let mut core = BookCore::init(code);
    core.get_book_detail(bid)
}

#[frb]
pub fn core_catalog(code:String,bid: String) -> Result<Vec<CatalogVolume>, String> {
    let mut core = BookCore::init(code);
    core.get_catalog(bid)
}

#[frb]
pub fn core_chapter(code:String,bid: String,cid: String) -> Result<Chapter, String> {
    let mut core = BookCore::init(code);
